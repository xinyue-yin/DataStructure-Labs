...
template <typename T>
void BinaryTree<T>::printLeftToRight(const Node* root) const {
  if(root!= NULL) {
...
template <typename T>
void BinaryTree<T>::printPaths (Node* root, vector<T> path) const {
  if (root!=NULL){
...
template <typename T>
int BinaryTree<T>::sumDistances(Node* root, int d) const{
  if (root==NULL){
    return 0;
  } else {
    d = d + sumDistances(root->left, d+1) + sumDistances(root->right, d+1);
    return d;
  }
}
/**
